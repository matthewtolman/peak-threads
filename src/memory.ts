...
export class Address<
  ...
  R = T extends BigIntTypedArray ? bigint : number,
> {
  private mem: T;
  private indx: number;
  private cnt: number;
   ...
   */
  static hydrate<T extends TypedArray>({
    memory,
    offset,
    cnt,
  }: DehydratedAddress<T>) {
   ...
   */
  static dehydrate<T extends TypedArray>(
    addr: Address<T>,
  ): DehydratedAddress<T> {
    return {
      memory: addr.memory(),
  ...
  public atomicAdd(amt?: R, index = 0): R {
    ...
    if (typeof amt === "undefined") {
      if (
        ...
        this.mem instanceof BigInt64Array
      ) {
        (amt as any) = 1n;
      } else {
        (amt as any) = 1;
    ...
    if (
      !(
        this.mem instanceof Int32Array ||
        this.mem instanceof BigInt64Array ||
        this.mem instanceof Int16Array ||
        this.mem instanceof Int8Array ||
        this.mem instanceof Uint32Array ||
        this.mem instanceof Uint16Array ||
        this.mem instanceof Uint8Array ||
        this.mem instanceof BigUint64Array
      )
    ) {
...
export function make<T>(Type: T, ...args: any[]) {
  ...
  const layouts: Array<
    [
      {
        new (_: any, _1?: number, _2?: number): TypedArray;
