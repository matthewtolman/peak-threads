...
import {
    type DefaultThread,
    Thread, type ThreadDef, type ThreadDefinition,
    type ThreadDefinitionWork,
    type ThreadWorker,
    type ThreadWorkInputValue,
    type ThreadWorkReturnValue, type ThreadWorkType
} from "./thread.ts";
import {
    ThreadClosedError,
...
/**
 * Options for setting up a thread pool
 */
export interface ThreadPoolOptions<D extends ThreadDefinition, T extends Thread<D>> {
    /**
...
interface ThreadInfo<D extends ThreadDefinition, T extends Thread<D>> {
 ...
 *
 * Type is inferred from the thread definition generic parameter. To set it to a specific type based on a specific Thread type,
 * use the `ThreadDef` helper (e.g. ThreadPool<ThreadDef<MyThread>>). Generally, this is used in conjunction with `Thread.serve`
 * to get a fully typed ThreadPool. Example:
 *
 * ```ts
 * // worker.ts
 * const threadDef = {
 ...
 * }
 *
 * export WorkerThread = Thread<typeof threadDef>
 * Thread.serve(threadDef)
 *
 * // pool.ts
 * import type {WorkerThread} from './worker.ts'
 * import WorkerUrl from './worker.ts?worker&url' // vite web worker import
 *
 * // spawn pool with type safety
 * const pool = ThreadPool.spawn<ThreadDef<WorkerThread>>(WorkerUrl)
 * ```
 */
export class ThreadPool<
    D extends ThreadDefinition = DefaultThread,
    T extends Thread<D> = Thread<D>
> implements ThreadWorker {
    private threads: ThreadInfo<D, T>[];
    private maxThreads: number;
    private minThreads: number;
    private lastLive: number;
    private options: ThreadPoolOptions<D, T>;
    ...
    private constructor(
        ...
        if (
            typeof options?.minThreads === "undefined" ||
            options?.minThreads === null ||
            options?.minThreads > maxCount
        ) {
        ...
        for (let i = 0; i < this.minThreads; ++i) {
            try {
                ...
                const close = async () => {
                    ...
                    this.threads[threadObj.indx].thread =
                        await this.threads[i].initPromise!!;
     ...
     */
    public static spawn<D extends ThreadDefinition = DefaultThread, T extends Thread<D> = Thread<D>>(
        script: string,
        options?: ThreadPoolOptions<D, T>,
    ): Promise<ThreadPool<D, T>> {
        return new Promise<ThreadPool<D, T>>((resolve, reject) => {
            const t = new ThreadPool<D, T>(
                () => resolve(t),
                (err: any) => reject(err),
                script,
                options,
    ...
    /**
     * Selects a thread to dispatch to
     * @private
     */
    private async selectThread(): Promise<T> {
        let attempt = 0;
        while (true) {
            try {
     ...
     * Will use the generic `onwork` handlers in the thread.
     ...
     */
    public async sendWork(work: ThreadWorkInputValue<ThreadDefinitionWork<ThreadDef<Thread>>>,
                          options?: StructuredSerializeOptions): Promise<ThreadWorkReturnValue<ThreadDefinitionWork<ThreadDef<Thread>>>>
    /**
     * Sends a job to the thread pool to be scheduled. May grow the pool if needed.
     * Will use the typed work handler with the work type corresponding to `type`.
     * **Note:** MUST provide both `type` and `work` to use this variant!
     *
     * @param type Work type to dispatch to
     * @param work Work to send
     * @param options Options for postMessage (be careful when transferring ownership! If a thread dies you may not be able to recover the transferred data!)
     */
    public async sendWork<Type extends ThreadWorkType<ThreadDefinitionWork<ThreadDef<Thread>>>>(
        type: Type,
        work: ThreadWorkInputValue<ThreadDefinitionWork<ThreadDef<Thread>>, Type>,
        options?: StructuredSerializeOptions,
    ): Promise<ThreadWorkReturnValue<ThreadDefinitionWork<ThreadDef<Thread>>, Type>>
    public async sendWork(
        typeOrWork: any,
        workOrOptions?: any,
        opts?: any,
    ) {
        ...
        for (let i = 0; i < maxAttempts; ++i) {
            ...
            try {
                return await thread.sendWork(typeOrWork, workOrOptions, opts);
            } catch (e: any) {
                // cannot retry, data is lost!
                if (opts?.transfer) {
                    throw e;
                }
                if (workOrOptions && 'transfer' in workOrOptions && workOrOptions?.transfer) {
                    throw e;
                }
    ...
    /**
     * Grows the pool by spawning a new thread
     * @private
     */
    private async growPool() {
        const i = ++this.lastLive;
        const threadObj: ThreadInfo<D, T> = {
            initPromise: undefined,
            indx: i,
        ...
        try {
            this.threads[i] = threadObj;
            this.threads[threadObj.indx].live = false;
            this.threads[threadObj.indx].initPromise = Thread.spawn(this.script, {
