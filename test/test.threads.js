...
describe("Thread", () => {
  ...
  it("transfer handler transfers ownership", async function () {
    ...
    ints.set([99], 0);
    const thread = await Thread.spawn("worker2.js", {
      initData: 45,
      onEventHandler: handler,
    });
    expect(thread).to.not.be.null;
  ...
  it("transfer handler transfers ownership - nested", async function () {
    ...
    const ints = new Int32Array(ab);
    ints.set([99], 0);
    const thread = await Thread.spawn("worker4.js", {
      initData: 45,
      onEventHandler: handler,
    });
    expect(thread).to.not.be.null;
  ...
  it("can transfer back", async function () {
    ...
    ints.set([99], 0);
    const thread = await Thread.spawn("worker3.js", {
      initData: 45,
      onTransferHandler: handler,
    });
    expect(thread).to.not.be.null;
...
describe("SharedThread", () => {
  ...
  it("transfer handler transfers ownership", async function () {
    ...
    const ints = new Int32Array(ab);
    ints.set([99], 0);
    const thread = await SharedThread.connect("shared-worker2.js", {
      initData: 45,
      onEventHandler: handler,
    });
    expect(thread).to.not.be.null;
  ...
  it("can transfer back", async function () {
    ...
    ints.set([99], 0);
    const thread = await SharedThread.connect("shared-worker3.js", {
      initData: 45,
      onTransferHandler: handler,
    });
    expect(thread).to.not.be.null;
...
describe("ThreadPool", () => {
  ...
  it("can create dynamic thread pool", async function () {
    console.info(this.test.parent.title + ".`" + this.test.title + "`");
    this.timeout(30_000);
    const pool = await ThreadPool.spawn("worker-slow.js", {
      initData: 2,
      minThreads: 0,
      closeThreadWhenIdle: 10,
    });
    expect(pool).to.not.be.null;
...
describe("ConditionVariable", async function () {
  it("can notify", async function () {
    ...
    const thread = await Thread.spawn("cond_var.js", {
      initData: { mux, cv, mem },
    });
...
describe("WaitGroup", async function () {
  it("can lock wait until tasks are done", async function () {
    ...
    const mem = new Int32Array(new SharedArrayBuffer(64));
    const thread = await Thread.spawn("wait_group.js", {
      initData: { wg, mem },
    });
...
describe("Barrier", async function () {
  it("blocks until all threads hit the barrier", async function () {
    ...
    const mem = new Int32Array(new SharedArrayBuffer(64));
    const thread1 = await Thread.spawn("barrier.js", {
      initData: { bar, mem },
      closeWhenIdle: 500,
    });
    const thread2 = await Thread.spawn("barrier.js", {
      initData: { bar, mem },
      closeWhenIdle: 500,
    });
...
describe("Semaphore", async function () {
  it("can lock when contended", async function () {
    ...
    for (let i = 0; i < 2; ++i) {
      ...
      const mem = new Int32Array(new SharedArrayBuffer(64));
      const thread1 = await Thread.spawn("semaphore.js", {
        initData: { sem, mem },
        closeWhenIdle: 200,
      });
      const thread2 = await Thread.spawn("semaphore.js", {
        initData: { sem, mem },
        closeWhenIdle: 200,
      });
      const thread3 = await Thread.spawn("semaphore.js", {
        initData: { sem, mem },
        closeWhenIdle: 200,
      });
      const thread4 = await Thread.spawn("semaphore.js", {
        initData: { sem, mem },
        closeWhenIdle: 200,
      });
...
describe("Mutex", async function () {
  it("can do basic locking", async function () {
    ...
    await mux.lockAsync();
    const thread1 = await Thread.spawn("mutex.js", {
      initData: { mux, mem },
      closeWhenIdle: 200,
    });
    const thread2 = await Thread.spawn("mutex.js", {
      initData: { mux, mem },
      closeWhenIdle: 200,
    });
  ...
  it("can lock when contended", async function () {
    ...
    const thread3 = await Thread.spawn("mutex.js", {
      initData: { mux, mem },
      closeWhenIdle: 200,
    });
